...
<<<<<<< HEAD:dev/0.2/src/js/three/j1.js
var finishedCollisionPoints = false;
=======
var koko = false;
>>>>>>> f25c6109ed3b75addfe31119813a0ce74a7cec38:dev/1.0/j1.js
...
var renderDiv = document.getElementById('renderCol');
...
var control = new THREE.TransformControls( camera, renderDiv);
...
function GenerateTunel() {
  if (firstF) {
    ...
    if (RayOrigin.y < -heightEmitter / 2) {
      firstF = false;
<<<<<<< HEAD:dev/0.2/src/js/three/j1.js
      finishedCollisionPoints = true;
    } else {
      RayOrigin.x += FluxStep;
    }
  } else if (finishedCollisionPoints) {
    if (nextF != nextFLimit) {
=======
      koko = true;
    } else {
...
function genConfigs(){
  if(genConfigsState){
    ...
    genConfigsState = false;
  }else{
    document.getElementById('configs').style.display = "block";
    genConfigsState = true;
  }
}

function applyGenConfigs(){
  console.log("aasdasd")
  nextFLimit = document.getElementById('nextFlow').value;
  FluxStep = document.getElementById('FluxStep').value;
}
